...
router.post('/', async (req, res) => {
    try {
        ...
        // Respond immediately — send emails fire-and-forget (non-blocking)
        res.status(201).json({ success: true, message: 'Message sent successfully!' });

        // Send emails if email system is configured (async)
        if (emailConfigured) {
            Promise.all([
                ...
                sendEmail({
                    to: process.env.SMTP_USER || process.env.EMAIL_USER || 'mallinadhanu@gmail.com',
            ...
            ]).then(() => console.log(`✅ Emails sent: Thank-you to ${email}, Notification to admin`))
              .catch(err => console.error(`❌ Contact email failed for ${email}:`, err.message));
        }
    } catch (err) {
